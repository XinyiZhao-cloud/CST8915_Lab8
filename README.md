...
I also configured a headless service (clusterIP: None), which gives each MongoDB pod a stable DNS name like mongodb-0.mongodb. This is required for setting up communication between the replicas.

To make the data persistent, I added PersistentVolumeClaims (PVCs) using volumeClaimTemplates, so each MongoDB pod has its own storage. The data is stored in /data/db, which means it is not lost even if a pod is deleted and recreated.

Then I initialized a replica set (rs0) manually using the MongoDB shell. After that, I verified it using rs.status(), which showed that all three nodes were part of the replica set. This setup improves availability because the system can continue working even if one node fails.
...
I used ChatGPT during this lab to help with debugging and understanding some Kubernetes concepts.
...
- Improve the writing in the README
